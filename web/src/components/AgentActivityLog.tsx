...
function LogIcon({ type }: { type: AgentLogEntry['type'] }) {
  ...
  switch (type) {
    ...
    case 'tool_call':
      return (
        <svg className={`${baseClass} text-foreground`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function AgentActivityLog({ entries, currentStatus, isStreaming }: AgentActivityLogProps) {
  ...
  const displayStatus = currentStatus || (latestEntry?.content) || '处理中...';
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-medium ${
                      ...
                      entry.type === 'tool_call' ? 'text-foreground' :
